...
/**
 * A {@code CallbackHandler} that prompts and reads from the command line for
...
public class ConsoleCallbackHandler implements CallbackHandler {
     ...
     * @throws UnsupportedCallbackException if the callback is not an instance of
    ...
    private String readLine() throws IOException {
        final String result = new BufferedReader(new InputStreamReader(System.in)).readLine();
        if (result == null) {
    ...
    private void doConfirmation(final ConfirmationCallback confirmation)
        ...
        for (int i = 0; i < options.length; i++) {
            if (optionType == ConfirmationCallback.UNSPECIFIED_OPTION) {
                ...
                System.err
                        .println(i + ". " + options[i].name + (options[i].value == defaultOption ? " [default]" : ""));
            }
        }
